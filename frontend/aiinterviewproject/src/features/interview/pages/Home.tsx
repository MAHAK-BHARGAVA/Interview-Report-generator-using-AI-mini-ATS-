...
function getDropzoneStyle(isDragOver: boolean, error?: string, hasFile?: boolean): CSSProperties {
  return {
    border: `1.5px dashed ${error ? tokens.errorBorder : isDragOver ? tokens.primary : hasFile ? tokens.primary : tokens.inputBorder}`,
    borderRadius: tokens.borderRadius,
    background: isDragOver ? tokens.primaryLight : hasFile ? "#F5F7FF" : tokens.inputBg,
    padding: "28px 20px",
    display: "flex",
    flexDirection: "column",
    alignItems: "center",
...
function CheckIcon() {
  return (
    <svg width="32" height="32" viewBox="0 0 24 24" fill="none"
      stroke={tokens.primary} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
  ...
  return (
    <nav style={navStyle}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 32 }}>
        <a href="#" style={navLinkStyle}>My Reports</a>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 24, paddingTop: 8 }}>
      <div>
        <h1 style={{
          ...
          Generate Your<br />Recruitment Match<br />Report
        </h1>
        <p style={{
          marginTop: 16,
          fontSize: 14,
          ...
          Upload your details to get an AI-powered analysis of your fit for the role.
          Our advanced algorithms analyze compatibility across skills, experience,
          and professional narrative.
        </p>
      </div>
      <div style={{ borderRadius: 12, overflow: "hidden", width: "100%", maxWidth: 420, aspectRatio: "4/3" }}>
        <img
...
export function JobDescriptionInput({ value, onChange, error }: JobDescriptionInputProps) {
  ...
  return (
    <div style={fieldWrapStyle}>
      ...
      />
      {error && <p id="job-description-error" style={errorMsgStyle}>{error}</p>}
    </div>
  );
}

// ─── SelfDescriptionInput ─────────────────────────────────────
export function SelfDescriptionInput({ value, onChange, error }: SelfDescriptionInputProps) {
  const [focused, setFocused] = useState<boolean>(false);

  return (
    <div style={fieldWrapStyle}>
      <label style={labelStyle} htmlFor="self-description">Self Description</label>
      <textarea
        id="self-description"
        value={value}
        onChange={(e: ChangeEvent<HTMLTextAreaElement>) => onChange(e.target.value)}
      ...
      />
      {error && <p id="self-description-error" style={errorMsgStyle}>{error}</p>}
    </div>
  );
}
...
export function ResumeUploader({
  file, isDragOver, onFileSelect, onDragOver, onDragLeave, onDrop, error,
}: ResumeUploaderProps) {
    ...
    <div style={fieldWrapStyle}>
      ...
      >
        ...
        {file ? (
          <>
            <CheckIcon />
            <p style={{ margin: "8px 0 2px", fontWeight: 600, fontSize: 14, color: tokens.primary }}>
              {file.name}
            </p>
            <p style={{ margin: 0, fontSize: 12, color: tokens.textMuted }}>Click to replace</p>
          </>
        ) : (
          <>
            <UploadCloudIcon isDragOver={isDragOver} />
            <p style={{ margin: "10px 0 2px", fontSize: 14, color: tokens.text, fontWeight: 500 }}>
              {isDragOver ? "Drop your PDF here" : "Click to upload or drag and drop"}
            </p>
            <p style={{ margin: 0, fontSize: 12, color: tokens.textMuted }}>.PDF format only</p>
          </>
        )}
      </div>
...
export function GenerateReportButton({ isLoading, isDisabled, onClick }: GenerateReportButtonProps) {
  return (
    <button
      onClick={onClick}
...
export function Footer() {
  const links: string[] = ["Privacy Policy", "Terms of Service", "Contact Support", "API Documentation"];

  return (
    <footer style={footerStyle}>
      <div>
        <p style={{ margin: 0, fontWeight: 700, fontSize: 13, color: tokens.text }}>RecruitAI</p>
      ...
      <nav style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
        {links.map((link) => (
          <a key={link} href="#" style={{ fontSize: 12, color: tokens.textMuted, textDecoration: "none" }}>
            {link}
          </a>
        ))}
      </nav>
